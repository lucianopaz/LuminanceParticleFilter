...
max_sigma = 30
for i,true_sigma in enumerate(np.linspace(5,max_sigma,n_sigmas)):
	print i,true_sigma
...
figure()
subplot(121)
imshow(mat_cloglike,aspect='auto',interpolation='none',extent=[-1,1,max_sigma,5])
xlabel('T [s]')
ylabel('$\sigma$')
cbar = colorbar()
cbar.set_label('Log likelihood')
subplot(122)
plot([-1,1],np.mean(dat[:,2])*np.ones(2),'-k')
for h,hs in zip(mat_cpredicted_hits,mat_cpredicted_hits_std):
	errorbar(np.linspace(-1,1,h.shape[0]), h, yerr=hs)
...
figure()
subplot(121)
imshow(mat_loglike,aspect='auto',interpolation='none',extent=[0,1,max_sigma,5])
xlabel('T [s]')
ylabel('$\sigma$')
cbar = colorbar()
cbar.set_label('Log likelihood')
subplot(122)
plot([0,1],np.mean(dat[:,2])*np.ones(2),'-k')
for h,hs in zip(mat_predicted_hits,mat_predicted_hits_std):
	errorbar(np.linspace(0,1,h.shape[0]), h, yerr=hs)
plt.gca().set_ylim([0,1])
xlabel('T [s]')
ylabel('Performance')
