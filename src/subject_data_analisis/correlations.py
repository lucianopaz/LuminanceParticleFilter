...
import enum, os, sys, math, scipy, pickle, cma, copy, re
...
for index,(s,fname) in enumerate(zip(subjects,fit_files)):
	...
	try:
		high_conf_threshold = fit[0]['high_confidence_threshold']
	except:
		fname2 = re.sub('_full_','_confidence_only_',fname)
		f2 = open(fname2,'r')
		fit = pickle.load(f2)
		f2.close()
		fit = fit['fit_output']
	...
	print 'nLL = {0}'.format(stats[s.id]['nLL'])
	print 'Subject stats:'
	...
	print '-----------------------------------'
f = open('fits/stats_inference_full_seconds.pkl','w')
pickle.dump(stats,f)
f.close()
