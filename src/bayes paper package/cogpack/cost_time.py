...
class DecisionPolicy():
	...
	def __init__(self,model_var=None,internal_var=0.,external_var=None,
		...
		if not self.known_variance():
			if prior_var_prob is None or self.model_var.shape!=prior_var_prob:
			...
			if isinstance(self.external_var,np.ndarray) and self.external_var.shape==self.model_var:
				self.external_var = self.external_var[inds]
			if isinstance(self.internal_var,np.ndarray) and self.internal_var.shape==self.model_var:
...
def test():
	d = DecisionPolicy(model_var=None,internal_var=10.,external_var=100.,
				 prior_mu_mean=0.,prior_mu_var=1000.,n=101,dt=1e-2,T=1.,
				 reward=1.,penalty=0.,iti=3.,tp=0.,cost=0.,discrete_prior=None,
				 prior_var_prob=None)
	xub,xlb = d.xbounds()
	first_passage_time = np.array(d.rt(mu=1.,bounds=(xub,xlb),model_var=d.model_var))
	
	hct = 0.3
	s = 1.7
	from matplotlib import pyplot as plt
	confidence_response = d.confidence_mapping(hct,s,confidence_mapping_method='belief')
	plt.figure()
	ax = plt.subplot(311)
	plt.plot(d.t,(d.bounds.T*2-1)*np.array([1,-1]))
	plt.ylabel('Normed Belief')
	plt.subplot(312,sharex=ax)
	plt.plot(d.t,confidence_response.T)
	plt.ylabel('Confidence response')
	plt.subplot(313,sharex=ax)
	plt.plot(d.t,first_passage_time.T)
	plt.ylabel('First passage time')
	
	fpt_conf = d.fpt_conf_matrix(first_passage_time,confidence_response,100)
	plt.figure()
	ax = plt.subplot(211)
	plt.imshow(fpt_conf[0].T,aspect='auto',cmap='jet',origin='lower',interpolation='none',extent=[d.t[0],d.t[-1],0,1])
	plt.plot(d.t,confidence_response[0],'--k')
	plt.subplot(212,sharex=ax)
	plt.plot(d.t,np.sum(fpt_conf,axis=-1).T)
	plt.plot(d.t,first_passage_time.T,'--')
	
	dead_time_convolver,conv_x = d.get_dead_time_convolver(0.2,0.4)
	rt_conf = d.rt_conf_matrix(first_passage_time,confidence_response,dead_time_convolver,100)
	t = np.arange(rt_conf.shape[1])*d.dt
	plt.figure()
	ax = plt.subplot(211)
	plt.imshow(rt_conf[0].T,aspect='auto',cmap='jet',origin='lower',interpolation='none',extent=[t[0],t[-1],0,1])
	ax = plt.subplot(212)
	plt.imshow(rt_conf[1].T,aspect='auto',cmap='jet',origin='lower',interpolation='none',extent=[t[0],t[-1],0,1])
...
def _test():
	...
	plt.figure()
	plt.plot(d1.t,rt1[0],'b',label='Conj')
	plt.plot(d1.t,rt1[1],'--b')
	plt.plot(d2.t,rt2[0],'g',label='Discrete mu')
	plt.plot(d2.t,rt2[1],'--g')
	plt.plot(d3.t,rt3[0],'r',label='Discrete var')
	plt.plot(d3.t,rt3[1],'--r')
	plt.legend(loc='best', fancybox=True, framealpha=0.5)
